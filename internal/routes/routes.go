...
func SetupRoutes(app *fiber.App, cfg *config.Config, userService *services.UserService, courseService *services.CourseService, testService *services.TestService, usersCourseService *services.UsersCourseService) {
	...
	app.Get("/test/:id", middlewares.RequireAuth(cfg, false), middlewares.RequireBought(cfg, usersCourseService, "test"), handlers.TestingPage(cfg, userService))
	app.Get("/course/:id", middlewares.RequireAuth(cfg, false), middlewares.RequireBought(cfg, usersCourseService, "course"), handlers.CourseViewPage(cfg, userService, courseService))
	app.Get("/homework", handlers.HomeworkPage(cfg, userService))
	app.Get("/policy", handlers.PolicyPage(cfg, userService))
	app.Get("/offer", handlers.OfferPage(cfg, userService))
